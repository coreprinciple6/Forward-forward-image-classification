...
class FF_MLP(nn.Module):  # , FF_Model_Skeleton):
    def __init__(self):
        ...
        self.conv1 = FF_Conv2D(1, 32, 3) 
        self.conv2 = FF_Conv2D(32, 64, 3) 
        self.fc1 = FF_Linear(9216, 10, nn.functional.relu)
    ...
    def get_layer_losses(self,
        ...
        for layer in self.layers:
            batch_pos, batch_neg, loss = layer.train_layer(
                batch_pos, batch_neg)
        ...
        for i in range(len(self.layers)):
            ...
            for cur_ep in range(epochs):
                for batch_pos, batch_neg in train_dataloader:
                    pos_out, neg_out, loss = layer.train_layer(
                        batch_pos, batch_neg)
                print("Layer", i+1, "Epoch", cur_ep, "Loss:", loss)

            batch_size = dataloader.batch_size
            updated_dataset = FF_Dataset((cur_layer(batch_pos), cur_layer(
                batch_neg)) for batch_pos, batch_neg in dataloader)

            train_dataloader = torch.utils.data.DataLoader(
                updated_dataset, batch_size=batch_size, shuffle=True)
...
class FF_Linear(nn.Linear):
    ...
    def forward(self,
        ...
        # Return f(W*X + b)
        # self.weight and self.bias part of nn.Linear
        return self.layer_activation(torch.mm(X, self.weight.T) + self.bias.unsqueeze(0))
    ...
    def train_layer(self,
        ...
        loss = self.layer_loss_fn(
            batch_pos_output, batch_neg_output, self.layer_loss_threshold)
...
def main():
    ...
    train_data_labeled = FF_Dataset(add_labels_to_data_fn(
        X.to(device), Y.to(device), False) for X, Y in train_loader)

    FF_train_loader = torch.utils.dataset.DataLoader(
        train_data_labeled, batch_size=train_loader.batch_size, shuffle=True)
